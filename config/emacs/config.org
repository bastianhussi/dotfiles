...
#+begin_src emacs-lisp :tangle yes
;; TODO: https://www.gnu.org/software/emacs/manual/html_node/emacs/Mouse-Avoidance.html
(use-package avoid
  :commands mouse-avoidance-mode
  :custom
  (mouse-avoidance-mode 'banish)
  :hook (window-setup . mouse-avoidance-mode))
...
(defun my/hl-todo ()
  ...
  (font-lock-add-keywords
   nil '(("\\(\\<\\(TODO\\)\\>[:]*\\)"       1 '((t :inherit success :weight bold)) t)
         ("\\(\\<\\(FIXME\\)\\>[:]*\\)"      1 '((t :inherit error :weight bold)) t)
         ("\\(\\<\\(HACK\\)\\>[:]*\\)"       1 '((t :inherit error :weight bold)) t)
         ("\\(\\<\\(DEBUG\\)\\>[:]*\\)"      1 '((t :inherit warning :weight bold)) t)
         ("\\(\\<\\(NOTE\\)\\>[:]*\\)"       1 '((t :inherit success :weight bold)) t)
         ("\\(\\<\\(SEE\\)\\>[:]*\\)"        1 '((t :inherit success :weight bold)) t)
...
(use-package whitespace
  ...
  (defun my/whitespace-write-contents ()
    (add-hook 'write-contents-functions #'whitespace-cleanup -90 t))
  :hook
...
(use-package recentf
  ...
  (recentf-auto-cleanup (* 60 60)) ; cleanup every hour
  :config
  (recentf-mode +1)
...
[[https://github.com/Somelauw/evil-org-mode/blob/master/evil-org-agenda.el][Evil Org Mode]]
#+begin_src emacs-lisp :tangle yes
...
(use-package eglot
  ...
  (defun my/eglot-write-contents ()
    (add-hook 'write-contents-functions #'eglot-format-buffer 90 t))
  :general
  (:states 'normal
...
(use-package prettier
  ...
  :config
  ; by setting the depth to 100 we're making sure
  ; that this will be run after eglot-format-buffer
  (defun my/prettier-write-contents ()
    (add-hook 'write-contents-functions #'prettier-prettify 10 t))
  :hook ((js-mode
          typescript-mode
          vue-mode
          mhtml-mode
...
(use-package desktop
  :commands (desktop-read desktop-save-mode)
  :custom
  ...
  ;; REVIEW: desktop-globals-to-clear
  ;; TODO: do not store dired, tramp, eglot
  :hook
