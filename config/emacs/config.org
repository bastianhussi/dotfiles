...
(use-package font-lock
  ...
  ;; FIXME: speed up colorization in org code blocks
...
(add-hook 'window-setup-hook #'global-prettify-symbols-mode)
#+end_src
...
#+begin_src emacs-lisp :tangle yes
;; REVIEW: remove `use-packge' here? If not any way to defer this?
(use-package newcomment
  :custom
  (comment-empty-lines t) ; don't comment empty lines
  (comment-fill-column nil)
  (comment-multi-line t)
  ;; continue comment when breaking the previous line
...
(use-package tab-bar
  ...
  ;; TODO: select tabs with number (`tab-bar-select-tab' X)
  (defhydra my/hydra-tab-bar (:color amaranth)
    ...
    ("RET" tab-bar-switch-to-tab "Select tab by name" :column "Navigation")
    ("l"   tab-next "Next Tab")
...
;; REVIEW: variables and other modes like `electric-quot-mode'
...
| =tramp= | Built-in |
#+begin_src emacs-lisp :tangle yes
;; REVIEW: is the usage of `use-package' really necessary?
(use-package tramp
  :custom
...
(use-package org
  ...
  ;; TODO: add all *.org-files from `org-directory'
  ...
  (my/leader-def
    ...
    "o"  '(:keymap org-mode-map :which-key "Org"))
  ;; TODO: keybindings
  :hook
...
(evil-define-key 'normal org-mode-map
  ...
  "H" 'org-shiftleft ; NOTE: rempas HIGH
  ; "J" 'org-shiftdown ; NOTE: this remaps `evil-join'
  "K" 'org-shiftup
  "L" 'org-shiftright ; NOTE: remaps LOW
...
(use-package writeroom-mode
  ...
  :hook
  (writeroom-mode-enable . my/writeroom-enable)
  (writeroom-mode-disable . my/writeroom-disable))
#+end_src

Emacs client for Music Player Daemon (MPD)

| =mpdel=     | [[https://github.com/mpdel/mpdel][Github]] |
| =ivy-mpdel= | [[https://github.com/mpdel/ivy-mpdel][Github]] |
#+begin_src emacs-lisp :tangle yes
;; TODO: defer loading this package until it will be actually used
(use-package mpdel
  :straight t
  :defer 10
  :custom
  (mpdel-prefix-key "") ; do not bind `mpdel' this way
  :config
  (mpdel-mode +1)
  :general
  (my/leader-def
    "m" '(:keymap mpdel-core-map :which-key "Listen")))
...
#+begin_src emacs-lisp :tangle yes
;; REVIEW: setup motion keymap for elfeed search like with org-agenda

;; https://regex101.com/
;; SEE: `regexp-builder' is very usefull

(setq browse-url-browser-function
      '(("https?:\/\/.*\..*\/.*\.\\(?:mp3\\|mp4\\|flac\\|ogg\\|wav\\)\\(?:\?.+\\)?"
         . my/browse-url-vlc)
        ("." . browse-url-default-browser)))

(defun my/browse-url-vlc (url &optional single)
  (call-process "vlc" nil 0 nil url))

;; (setq browse-url-generic-program ...) mail-user-agent

;; TODO: map go to visual mode and open all feeds
...
(use-package elfeed
  ...
  ;; since we use an additional window to display the feed with efleed-show-mode
  ;; wee need to clean this window up when leaving the buffer
...
(use-package mu4e
  ...
  ;; Defer loading this package by 30 seconds.
  ...
  ;; TODO: review this variable and improve auto-formatting mail
  ...
  (mu4e t)) ;; Sync emails in the background when this package gets loaded
#+end_src


#+begin_src emacs-lisp :tangle yes
(add-hook 'mu4e-compose-mode-hook
    (defun my/mu4e-compose ()
...
;; REVIEW: need to adjust `evil-shift-width'?
(use-package js
  :config
...
;; TODO: find a suiting solution for tsx-files
#+end_src
...
(defhydra my/hydra-buffers (:color amaranth)
  "Buffer management"
  ("h" switch-to-next-buffer "Next" :column "Navigate")
  ("l" switch-to-prev-buffer "Previous")
  ("w" switch-to-buffer-other-window "Window")
  ("t" switch-to-buffer-other-tab "Tab")
  ("F" switch-to-buffer-other-frame "Frame")
  ("f" counsel-fzf "Fzf" :column "Find" :exit t)
  ("g" counsel-rg "Grep" :exit t)
  ("b" counsel-switch-buffer "Buffers" :exit t)
  ...
  ("o" my/kill-other-buffers "Other" :exit t)
  ("a" my/kill-all-buffers "All")
  ("q" nil "Quit" :column "Exit" :exit t))
#+end_src
...
(my/leader-def
  ...
  "dd"  '(my/desktop-delete :which-key "Delete")
  "ds"  '(desktop-save :which-key "Save")
  "dR"  '(desktop-revert :which-key "Revert")
  "s"   '(:ignore t :which-key "Server")
  "ss"  '(server-start :which-key "Start")
  "sd"  '(server-force-delete :which-key "Delete")
  ;; "u"  '(straight-* :which-key "Update")
  "l"   '(:ignore t :which-key "Launch")
  "lm"  '(mu4e :which-key "Mail")
  "lb"  '(elpher-go :which-key "Browser")
  "lf"  '(elfeed :which-key "Feeds")
  "lt"  '(vterm :which-key "Terminal")
  "le"  '(eshell :which-key "Eshell")
  "f"   '(my/hydra-buffers/body :which-key "Buffers")
  "b"   '(:keymap bookmark-map :which-key "Bookmarks")
  "q"   '(:ignore t :which-key "Quit")
  "qq"  '(save-buffers-kill-terminal :which-key "This Frame")
  ;; TODO: delete all frames
  "qo"  '(delete-other-frames :which-key "Other"))
#+end_src
...
| =desktop= | Built-in |
#+begin_src emacs-lisp :tangle yes
;; TODO: only use when there was no server already running
...
(use-package desktop
  ...
  (setq history-length 250) ;; SEE: (put 'evil-ex-history 'history-length 50)
