...
(use-package epa-file
  ...
  (epg-pinentry-mode 'loopback) ;; use the minibuffer instead of external program
...
(use-package vterm
  ...
  (defun my/vterm-adjust-evil-cursor ()
    "Prevent evil-mode from changing the appearance of the curosor inside different modes."
...
#+begin_src emacs-lisp :tangle yes
;; NOTE: luckily this package also prevents whitespace-style beeing shown inside company.
...
(use-package prettier
  ...
  :hook
  ((sgml-mode css-mode js-mode typescript-mode vue-mode yaml-mode json-mode)
   . prettier-mode))
#+end_src
...
#+begin_src emacs-lisp :tangle yes
;; split only if the original window has at least that many lines
(setq split-height-threshold 50
      split-width-threshold 125)

;; TODO: prevent Emacs from creating new frames
...
(setq display-buffer-base-action
  '((display-buffer-reuse-window
    ...
    . ((inhibit-same-window . t) ;; allow using the same window.
       (resuable-frames . nil)
       (preserve-size (t . t)) ;; preserve the width and height of the chosen buffer (must already exist)
       (inhibit-switch-frame t) ;; prevents another frame from being raised or selected
       ;; display-buffer-reuse-mode-window
       (mode . (help-mode magit-mode eldoc-mode ediff-mode)))))
#+end_src
